...
public class CS2420ClassGenericTester {
	...
	// Small and large CS2420 class tests
	// -------------------------------------------------------------------------

	@Test
	public void sortSmallClassByName() {
		assertEquals("Doe", verySmallClass.getOrderedByName().get(0).getLastName());
		assertEquals("Hall", verySmallClass.getOrderedByName().get(1).getLastName());
		assertEquals("Nguyen", verySmallClass.getOrderedByName().get(2).getLastName());
	}

	@Test
	public void sortSmallClassByUNID() {
		assertEquals(1010101, verySmallClass.getOrderedByName().get(0).getUNID());
		assertEquals(2323232, verySmallClass.getOrderedByName().get(1).getUNID());
		assertEquals(4545454, verySmallClass.getOrderedByName().get(2).getUNID());
	}

	@Test
	public void sortSmallClassByScore() {
		assertEquals(1010101, verySmallClass.getOrderedByName().get(0).getUNID());
		assertEquals(2323232, verySmallClass.getOrderedByName().get(1).getUNID());
		assertEquals(4545454, verySmallClass.getOrderedByName().get(2).getUNID());
	}
	...
	@Test
	public void sortLargeClassByName() {
		ArrayList<CS2420StudentGeneric<PhoneNumber>> nameList = largeClass.getOrderedByName();
		for (int i = 1; i < nameList.size(); i++) {
			String lhsLastName = nameList.get(i).getLastName();
			String rhsLastName = nameList.get(i - 1).getLastName();
			int lhsValue = lhsLastName.charAt(0);
			int rhsValue = rhsLastName.charAt(0);
			assertTrue(lhsValue >= rhsValue);
		}
	}

	@Test
	public void sortLargeClassByUNID() {
		ArrayList<CS2420StudentGeneric<PhoneNumber>> nameList = largeClass.getOrderedByUNID();
		for (int i = 1; i < nameList.size(); i++) {
			int lhsValue = nameList.get(i).getUNID();
			int rhsValue = nameList.get(i - 1).getUNID();
			assertTrue(lhsValue >= rhsValue);
		}
	}

	@Test
	public void sortLargeClassByScore() {
		ArrayList<CS2420StudentGeneric<PhoneNumber>> nameList = largeClass.getOrderedByScore(60.0);
		for (int i = 1; i < nameList.size(); i++) {
			double lhsValue = nameList.get(i).computeFinalScore();
			double rhsValue = nameList.get(i - 1).computeFinalScore();
			assertTrue(lhsValue <= rhsValue); // Score is listed in ascending order
		}
	}

	@Test
	public void testCalcLargeStudentFinalScore() {
		CS2420StudentGeneric<PhoneNumber> student = largeClass.lookup(new PhoneNumber("801-555-1234")).get(0);
		student.addScore(86.5, "assignment");
		student.addScore(75, "exam");
		student.addScore(100, "lab");
		student.addScore(89.2, "quiz");
		assertEquals(82.085, student.computeFinalScore(), 0);
	}
